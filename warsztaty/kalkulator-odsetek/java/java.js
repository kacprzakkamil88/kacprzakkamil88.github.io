// ta funkcja została zapisana na końcu po var podatek//
...
function obliczZysk(wplata, iloscLat, okresKapitalizacji, oprocentowanie, podatek) {
    if(podatek === true) {
        ...
        var oprocentowanieNieOpodatkowane = oprocentowanie;
    }
    
    var kapital = wplata * Math.pow(1 + oprocentowanieOpodatkowane / okresKapitalizacji, iloscLat * okresKapitalizacji);
    
    return parseFloat(kapital).toFixed(2); // to coś daje miejsca po przecinku
}

function policz(event) {
    event.preventDefault();
    var wplata = 
        parseInt(document.getElementById("wplata").value, 10);
